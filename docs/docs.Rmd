...
docs_files <- 
  ...
  mutate(
    doc_file = 
      file.path(target_folder, str_replace(source_file, "\\.[^.]+$", ".html")),
    source_file_hash = tools::md5sum(source_file)
  ) 

# docs hash
if (file.exists(hash_file)) {
  docs_hash <- read_csv(hash_file, col_types = "cccccc")
} else {
  docs_hash <- tibble(
    type = character(),
...
docs_files <- 
  docs_files %>% 
  left_join(
    rename(docs_hash, existing_source_file_hash = source_file_hash) %>% select(-source_file), 
    by = c("doc_file", "type")
  ) %>% 
  mutate(
    # generate if
...
render_file <- function(generate, type, source_file, target_folder, doc_file) {
  ...
  # copy source file
  file.copy(source_file, to = target_folder, overwrite = TRUE)
